defmodule Bauer.Producer do
  ...
  start at sending Kafka messages.
      ...
      iex> MyApp.MyProducer.produce(%Bauer.Message{value: "message value"})
  ...
  the message, the option topic will always take priority.
        ...
        MyApp.MyProducer.produce(%Bauer.Message{}, topic: "unique")
  ...
  need to be in the form of a `%Bauer.message()`. This can be done in
  ...
  defmacro __using__(opts \\ []) do
    quote do
      ...
      @producer_backend_module Keyword.get(@opts, :producer_backend, Bauer.SyncProducerBackend)
      ...
      `use Bauer.Producer`, however you can override this by setting the `topic`
      option.
      """
      ...
      """
      @spec prepare(Bauer.topic(), any(), keyword()) :: Bauer.Message.t()
